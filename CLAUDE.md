...
| `lib/formatTimeAgo` (`formatNumber`, `formatDayMonth`, `formatTimeAgo`, `formatBirthDate`, `formatFullDate`) | Formateo de números y fechas sin Intl — usar siempre estas utilidades propias |
| @react-native-community/datetimepicker@8.6.0 | Date picker nativo en Android (diálogo calendario) e iOS (spinner inline) — versión exacta sin caret |
...
- `getTitleTrophies()` y `getUserTrophiesEarnedForTitle()` devuelven `trophyEarnedRate` (string, ej. `"2.9"` = 2,9 % de jugadores) y `trophyRare` (bucket Sony: 0=Ultra Rara, 1=Muy Rara, 2=Rara, 3=Común) — campos expuestos por la librería, pero **no viable en la práctica**: verificado en la implementación de F46 (2026-07-08) que la API PS5 (v2) requiere contexto de un usuario que haya jugado el título para poblar `trophyEarnedRate`; con las credenciales del sistema del backend llega sistemáticamente `null`/`NaN` (486 trofeos comprobados, 0 poblados). PSN usa por eso XP por tipo de trofeo (Opción A, ver "Sistema de XP" más abajo). Ver T134 en BACKLOG.md para investigar fuentes alternativas si se retoma la rareza real de PSN.
...
### Rendimiento del sync — estado actual (T113/T114 resueltos, 2026-07-17)

El sync **ya no tarda ~50 min** en bibliotecas grandes — ese síntoma (T113) quedó resuelto de raíz por el batching de escrituras (T114). Medido en PSN con 379 juegos: **~27 min → ~1:43 (~16x)**.

- **Patrón de batching (los 3 adapters — RA, PSN, Steam)**: el cálculo (puntos normalizados, rareza, `trophyType`) se sigue haciendo en TypeScript exactamente igual que antes; lo que cambió es la persistencia — en lugar de upserts secuenciales por logro/trofeo (2 queries × N logros, en serie), cada juego persiste sus logros con `INSERT ... ON CONFLICT` en **2 fases**: (1) `Achievement` (rareza/puntos/trophyType en `DO UPDATE`), (2) `UserAchievement`. Una sola operación SQL por fase en lugar de N. Dedup por `externalId`. Aislamiento de fallo por juego intacto en los 3 adapters (Steam lo ganó en T139 — antes era secuencial sin `try/catch`, un juego que fallaba abortaba los siguientes del batch).
- **Por qué se descartó primero la hipótesis de "la red es el cuello de botella"**: instrumentación (`fetchMs`/`writeMs`/`totalMs` por título, sin cambios de lógica) reveló que `totalWriteMs` domina `totalFetchMs` **24:1** — el problema real eran las escrituras a Postgres, no las llamadas a PSN/Steam/RA. Ver DECISIONS.md ("Medir antes de optimizar").
- **`getMyGames` pagina en SQL real** (`$queryRaw`, commit `f460384`) — ya no trae todo el historial del usuario y hace `slice()` en memoria. Agregados y lista cacheados en Redis reusando `invalidateUserPublicCache`. Esto resuelve el tiempo de *lectura* de la biblioteca (independiente del tiempo de *sync*, que resolvió T114).
- **`processSyncJob`** (`sync.worker.ts`) es el processor real de BullMQ, extraído a función exportada (T140) — permite testearlo directamente contra Redis real sin pasar por el mock de `Worker`.
- **Convergencia de plataformas en `sync-bg-{userId}`**: si el job ya existe en estado `WAITING` cuando llega un 2º `.add()` (p. ej. cron excluye Steam por cuota y luego app-open lo re-añade), `enqueueOrMergeSyncBatch` (`sync.queue.ts`) mergea el array de plataformas en lugar de descartar el payload nuevo (bug T141, ya corregido). No-op si el job está `active`/`completed` — no se puede modificar un sync en curso sin tocar el worker.
...
- **`authenticate.ts` es fail-CLOSED ante fallo de BD, nunca fail-open** (T135, 2026-07-12): el check de soft-delete (`deletedAt: null`) tras verificar el JWT reintenta hasta 3 veces (`findActiveUserWithRetry`, backoff 50ms/150ms) para absorber hipos transitorios de BD; si los 3 intentos fallan, deniega con 503 `AUTH_CHECK_FAILED` y nunca rellena `req.user`. Un fallo persistente se reporta con `logger.error` + `Sentry.captureException` — nunca debe quedar mudo. No reintroducir el patrón `.catch(() => next())` en este middleware.
...
- **`router.back()` — prohibición absoluta**: **NO debe existir ningún `router.back()` crudo en todo el proyecto** fuera del propio `hooks/useSafeBack.ts`. Toda navegación hacia atrás usa el hook: `const safeBack = useSafeBack(); ... onPress={safeBack}`. Aplica a pantallas, componentes, y callbacks de Alert — sin excepciones. Verificación binaria: `grep -rn "router\.back()" apps/mobile/app apps/mobile/components apps/mobile/hooks` → resultado esperado: **0 líneas** (excepto dentro de `useSafeBack.ts`). Razón: sin historial en el stack (entrada por deep link, push notification o `router.replace` desde onboarding) `router.back()` saca al usuario de la app — regresión documentada como BUG-02 en vinculación de plataformas (2026-06-25), BUG-009 a BUG-014 (2026-06-26) y Lote 2b (2026-06-26, 7 usos legacy migrados). El hook llama `router.back()` si hay historial, o `router.replace('/(tabs)')` si no lo hay. **Nunca** escribir `router.back()` ni `if (router.canGoBack()) router.back()` inline — usar siempre el hook. El coste es cero: `useSafeBack` funciona en pantallas y componentes por igual.
- **Formateo de números y fechas**: usar siempre las utilidades propias de `lib/formatTimeAgo` (`formatNumber`, `formatDayMonth`, `formatTimeAgo`, `formatBirthDate`, `formatFullDate`). **Nunca** usar `Intl.NumberFormat`, `Intl.DateTimeFormat`, `Intl.RelativeTimeFormat`, `Number.prototype.toLocaleString()`, `Date.prototype.toLocaleDateString()` ni `Date.prototype.toLocaleTimeString()` — crash documentado en Sentry: `Intl.RelativeTimeFormat` es `undefined` en algunos dispositivos Android con el build actual de Hermes, lo que demuestra que el soporte Intl no está garantizado en todos los builds de Hermes aunque la documentación lo indique. `toLocaleString()` y `toLocaleDateString()` pueden apoyarse en Intl internamente y sufrir el mismo crash. **La regla aplica a TODOS los archivos de `apps/mobile/` sin excepción**, incluidos componentes, pantallas, tests y **código detrás de feature flags** — un Intl gateado sigue siendo un crash latente que explota al activar el flag. Para verificar antes de cada commit: `grep -rn "toLocaleString\|toLocaleDateString\|toLocaleTimeString\|new Intl\." apps/mobile/app apps/mobile/components apps/mobile/hooks apps/mobile/lib` — resultado esperado: 0 líneas (excepto comentarios y el propio `lib/formatTimeAgo.ts`). Antecedente: el barrido de BUG-01 (2026-06-25) eliminó 5 usos de `Intl.*` pero no detectó 6 usos adicionales de `Number.prototype.toLocaleString()` y `Date.prototype.toLocaleDateString()` en pantallas activas — corregidos en auditoría de 2026-06-26. BUG-007/008 (2026-06-27): 2 usos adicionales detectados en componentes gateados (`PremiumBanner.tsx`, `challenges.tsx`) — corregidos antes de que se activaran los flags. El grep debe dar 0 también en código gateado.
...
Flujo completo desde 2026-07-10 — Postgres + Redis en Docker local, aislado de producción por defecto. Ver [docs/BUILD_LOCAL.md](docs/BUILD_LOCAL.md) parte 1 para el detalle paso a paso.

- `apps/api/.env` (local, ignorado por git): Docker local (`localhost:5432`/`localhost:6379`), secretos de dev propios, sin credenciales de plataformas.
- `apps/api/.env.ops` (operaciones sobre producción, ignorado por git): credenciales reales de Railway — **nunca se carga por defecto**, solo con `--env-file=.env.ops` explícito (scripts de `scripts/backfill-*`, ver T133 en BACKLOG).
- `npm run dev:api` / `npm run dev:worker` (nuevo) usan `--env-file-if-exists=.env` — nunca rompe producción (Railway no tiene `.env`, usa variables nativas).
- **🔴 T135 en BACKLOG — investigar prioritario (hallazgo de seguridad)**: correr `npm test --workspace=apps/api` con Docker local levantado + `.env` presente hace fallar 21 tests de `user.routes.test.ts` — el test no mockea `lib/prisma` y pasaba por accidente del entorno (sin BD alcanzable, se activaba un camino "fail-open" en `authenticate.ts`). Dos problemas anidados: **(1)** si ese fail-open aplica a la autenticación en sí (no solo al chequeo de soft-delete), es un agujero de seguridad real — la norma en auth es fail-CLOSED; pendiente de investigar la gravedad exacta antes de nada. **(2)** los 21 tests verificaban la razón equivocada, no el comportamiento correcto. Ver T135 para el plan completo.
...
- **OTA (expo-updates): NO activo en producción** — el AAB en Producción actual (versionCode 12, vigente) fue generado sin `expo-updates`. v1.4.1 (versionCode 13) ya está compilado, subido a closed testing y con la promoción a Producción **solicitada** a Google (pendiente de aprobación, no de acción del desarrollador) — tampoco incluye `expo-updates` todavía. Hasta que se active OTA, cualquier fix de JS/assets requiere nuevo AAB. Ver OP07 en BACKLOG.md para los pasos exactos de activación.
...
22. ✅ Smoke tests de producción — PL19 (3/3 ✅: sync en tiempo real, navegación `useSafeBack`, edge-to-edge Android 15), verificado en Railway env production (sesión 2026-06-27)
23. ✅ **Play Store — EN PRODUCCIÓN.** versionCode 9 promovido a Producción el 2026-06-27 (segunda solicitud aprobada por Google) tras PL13 (limpieza BD) y PL14 (edge-to-edge) verificados. Versiones posteriores (10, 11, 12) desplegadas como actualizaciones normales de la app ya en producción — **versionCode 12 es el vigente hoy**. **v1.4.1 (versionCode 13)** compilado, subido a closed testing, con la promoción a Producción **solicitada** a Google — pendiente de aprobación (sin acción del desarrollador pendiente, solo espera).
...
**Fecha**: 2026-07-12 a 2026-07-18 (saga rendimiento del sync + tests reales contra BullMQ/Redis — ver `docs/SESSION_LOG.md` para el detalle completo por commit) — Unidad de trabajo continua desde el fix de seguridad T135 hasta el cierre de aislamiento Steam T139. Resumen: **(1)** T135 — `authenticate.ts` fail-open corregido a fail-closed con reintento (3 intentos) + logging Sentry; corrige los 21 tests de `user.routes.test.ts` que pasaban por accidente de entorno (`1aaf88b`); **(2)** T136 — regresión del tag de Platino (roto por F46 al recalibrar XP 300→100 mientras la detección hardcodeaba `===300`) corregida con `Achievement.trophyType` persistido + detección robusta con fallback transitorio (`2dc0c86`); **(3)** T143 — `getMyGames` pagina en SQL real (`$queryRaw`) en lugar de traer todo el historial y hacer `slice()` en memoria (`f460384`); **(4)** T114 — sync lento (T113) resuelto de raíz: instrumentación reveló que las escrituras a Postgres dominan la red 24:1 (hipótesis original errónea), batching `INSERT ... ON CONFLICT` en 2 fases desplegado por rollout gradual en los 3 adapters (RA `5cf4023` → PSN `cc2e591` → Steam `400fd2d`) — PSN 379 juegos medido ~27min → ~1:43 (~16x); **(5)** T129 — plan de cobertura de tests completado: TESTS-2 (#1-4, lifecycle "segundo sync" contra Redis real), TESTS-3 (reglas de negocio + aislamiento e2e por plataforma), TESTS-4 (centinelas contra la función real, no el mock) — `77139e5`/`3d8a7b1`/`d1fb31b`/`603e08e`; TESTS-5 (E2E) descartado deliberadamente, coste>valor; **(6)** T140 — processor de `sync.worker.ts` extraído a `processSyncJob` exportada, refactor puro que desbloqueó los tests de handler real (`5eec903`); **(7)** T141 — bug de convergencia destapado por TESTS-3 (2º `.add()` en `WAITING` descartaba el payload) corregido con `enqueueOrMergeSyncBatch`, variante JS robusta ante upgrades de BullMQ (`f0e4a16`); **(8)** T139 — Steam gana aislamiento de fallo por juego (patrón ya usado por RA/PSN), corrige además un bug de conteo latente (`2173d7c`). **Ejemplo de "los tests reales destapan bugs que los mocks esconden"**: T141 fue encontrado por TESTS-3 corriendo contra BullMQ real — la suite mockeada nunca lo habría detectado porque el mock de `syncQueue.add` siempre resuelve sin ejecutar la lógica de convergencia real de BullMQ. BACKLOG actualizado: T113/T114/T129/T135/T136/T139/T140/T141/T143 marcados ✅, TESTS-1 a TESTS-5 completos, T142 renombrado (duplicaba el ID de T136), T137/T130 siguen abiertos como deuda documentada. DECISIONS.md ampliado con 6 decisiones nuevas (medir antes de optimizar, SQL crudo vs `$transaction`, patrón de conteo T139, variante JS de T141, extracción pura T140, TESTS-5 descartado) + corrección de la decisión de fail-open (ya obsoleta tras T135). **Tests: 745 API · 497 mobile** (verificados en esta sesión de consolidación de docs — cifra final del rango, ver commits individuales en SESSION_LOG para el detalle incremental).

**Fecha**: 2026-07-06 a 2026-07-10 (sesión consolidada — ver `docs/SESSION_LOG.md` para el detalle completo por commit) — Resumen: **(1)** bug crítico de auto-bloqueo del sync automático resuelto y verificado (`removeOnComplete: true` en lugar de `{ count: N }` con jobId determinista — T128, commit `361dbd8`); **(2)** T127 (retar a un amigo por logro) resuelto — iconos + confirmación + haptics (`268cada`); **(3)** T125 resuelto (cron incluye `lastSyncAt = null`) + nueva alerta Sentry cuando expira el NPSSO del sistema, dedup 6h (`90049bb`); **(4)** T115 resuelto — el email de reset de contraseña ahora enlaza a un endpoint `https://` que redirige al deep link, en lugar del deep link directo que los clientes de email no abrían (`838a4f3`); **(5)** TESTS-1 implementado — suite de integración BullMQ contra Redis real (`8f4f441`), residual de teardown documentado en T130; **(6)** dos bugs de rewarded ad corregidos — cuelgue en "Cargando" sin listener `ERROR` + fuga de puntos al saltar el anuncio (recompensa ahora solo tras `EARNED_REWARD`, T131, `2d98c58`); **(7)** release v1.4.1/versionCode 13 (`f7ae0e4`), subido a closed testing con producción **solicitada** a Google; **(8)** F46 implementado en fases 1-2 + pivote a Opción A (`372778a`/`610d529`/`ff079f1`) — PSN calcula XP por tabla de tipo de trofeo (no rareza real, `psn-api` no la expone de forma fiable — 486 trofeos comprobados, 0 con el campo poblado), fase 3 (recálculo histórico) pendiente; **(9)** entorno de desarrollo local montado y aislado de producción — `apps/api/.env` (Docker local) vs `.env.ops` (Railway, requiere invocación explícita), scripts `dev` migrados a `--env-file-if-exists`, `docs/BUILD_LOCAL.md` ampliado (`2f72c08`). **Hallazgo colateral (T135 en BACKLOG)**: con Docker local + `.env` presentes, 21 tests de `user.routes.test.ts` fallan por una fragilidad de aislamiento preexistente (no mockean `lib/prisma`, dependían del fail-open de `authenticate.ts` cuando la BD era inalcanzable) — no es una regresión de código, queda documentado como deuda técnica, sin fix aplicado (sesión solo-docs). BACKLOG actualizado: T115/T125/T127/TESTS-1 marcados ✅; nuevos T132 (fragilidad Zod `CLOUDINARY_URL=""`), T133 (scripts backfill necesitan `.env.ops` explícito), T134 (F46 Opción B futura — investigar rareza PSN por otra vía), T135 (hallazgo de arriba). DECISIONS.md ampliado con 4 decisiones (F46 Opción A, corrección de la premisa de rareza PSN del 25-jun, recálculo de XP en cada sync, inversión del default local/producción). **Tests: 726 API · 497 mobile** (verificados en esta sesión de consolidación de docs — los 726 API incluyen los 21 fallos de T135 solo si Docker local está activo durante el run).

**Fecha**: 2026-07-08 (feat(scoring): F46 fase 3 — PSN pivota a Opción A, curva de rareza confirmada no viable) — Verificado en esta sesión que `psn-api` no devuelve `trophyEarnedRate` de forma fiable (limitación ya anotada en la sección PSN de este documento) — se descarta la Opción B (rareza real para PSN) para el lanzamiento y se confirma Opción A: PSN calcula XP por tipo de trofeo. `achievement-points.ts`: nueva `normalizePsnAchievementPoints(rarityPercent, trophyType)` — si `rarityPercent` es válido usa la misma curva que Steam (`normalizeAchievementPoints`, costura lista para cuando/si llega rareza real de PSN por otra vía); si no (el caso siempre hoy), fallback a tabla por tipo recalibrada: Bronce 10/Plata 20/Oro 50/Platino 100 (antes 15/30/90/300 — el valor viejo de Platino excedía el techo de la curva Steam, 150, descompensando el ranking cross-plataforma). `psn.adapter.ts`: los 2 call-sites de `normalizeAchievementPoints` migrados a `normalizePsnAchievementPoints(rarityValue, t.trophyType)`. Steam sin cambios (ya usaba rareza real desde F46 fase 1). Tests: 12 nuevos en `achievement-points.test.ts` (fallback por tipo, rareza fuera de rango, y la rama de rareza real para la costura futura); tests existentes de `psn.adapter.test.ts` sin cambios (usan mocks con rareza válida, delegan igual a la curva). Fórmula documentada en la sección "Sistema de XP" de este documento. **Tests: 726 API (+12) · sin cambios mobile · 0 TS/lint.** ~~No desplegado (solo develop)~~ — **corrección (2026-07-10)**: Railway hace auto-deploy en cada push a `develop` (ver OP02/OP05 en BACKLOG.md — comportamiento activo, no una decisión pendiente de tomar) y este commit sí se pusheó a `origin/develop`, así que **F46 fases 1-2 + Opción A están desplegadas en Railway producción**, no solo en el repo. La nota original no tuvo en cuenta el auto-deploy vigente.
...
**Fecha**: 2026-07-02 (docs — T122/T123 bugs v1.4.0 closed testing + sesión 2026-07-02) — Solo docs, 0 código. Prueba en dispositivo real del build v1.4.0 (closed testing). **T101 gate D(b) ✅ VERIFICADO**: clave `sync:appopen:{userId}` creada con TTL correcto (2331s observado ≈ cooldown 3600s/free); hook `useAutoSync` dispara el endpoint al abrir, encola el batch y aplica cooldown server-side por tier. T101 CERRADO Y VERIFICADO COMPLETO (cron ✅ + manual ✅ + app-open ✅). **Destapados durante prueba cerrada**: **T124** (nuevo, 🔴) — `queueInitialSync` (sync al vincular plataforma) no convergió en `sync-bg-{userId}` en el refactor T101; cabo suelto — sigue single-platform con jobId auto-generado → compite por lock → abandona → causa raíz de T122a. **T122** (preexistente, actualizado) — mutaciones core que el backend procesa pero la UI no refleja; síntoma (a) actualizado con CAUSA RAÍZ = T124. **T123** (preexistente, confirmado) — carga lenta de bibliotecas con ~375 juegos PSN por paginación client-side completa. Estado de plataformas del dev limpiado a solo PSN tras testing de vinculación RA repetida. Sin cambios de código. Tests: **673 API · 487 mobile** (sin cambios).

**Fecha**: 2026-06-27 (fix T101 — sync reencolado en bucle por lock por usuario) — Bug de producción cortado con restart del worker (2026-06-27). Causa raíz: `sync.worker.ts` reencolaba incondicionalmente cuando el lock Redis `sync:user-lock:{userId}` estaba ocupado — sin techo, sin dedup. Con usuarios PSN+RA, el scheduler nocturno encolaba un job por plataforma; ambos competían por el mismo lock; el segundo entraba en bucle de reencolados cada 5 s. Fix en 5 archivos, 5 commits en `develop` (`19d8f0a`–`7249e4a`). **sync.queue.ts**: `SyncBatchJobData` (array de plataformas), `lockRetryCount?` en `SyncJobData`, unión `AnySyncJobData`, `isBatchJob()` type guard. **background-sync.scheduler.ts**: agrupa cuentas por userId → UN job `SyncBatchJobData` por usuario con jobId determinista `sync-bg:{userId}` (deduplicación nativa BullMQ); Steam omitido del array por usuario cuando supera el 80 % de cuota (A41 mantenido). **sync.worker.ts**: branch batch — un lock Redis + `extendLock` entre plataformas en serie + abandono limpio si lock ocupado; `syncPlatform()` extraído como helper. Branch single-platform — failsafe `lockRetryCount` con techo `LOCK_MAX_RETRIES=3` × delay 30 s → abandona con `logger.warn` al alcanzarlo. **sync.service.ts**: `triggerManualSync` lee `sync:user-lock:{userId}` ANTES de consumir cooldown o cuota; devuelve `{status:'in_progress'}` si el lock existe (HTTP 202 — sin efecto en UX porque el socket ya emite progreso). **sync.controller.ts**: maneja el caso `status:'in_progress'` separado del caso `skippedByQuota`. **Tests**: `background-sync.scheduler.test.ts` nuevo (9 tests — agrupación, array de plataformas, jobId determinista, cuota Steam); `sync.worker.test.ts` +7 tests (failsafe techo/reencola/primer intento; batch lock+extendLock+fallo independiente por plataforma+lock ocupado abandona); `sync.service.test.ts` +2 tests in_progress + import `syncQueue` añadido. **Tests: 651 API (+18) · 462 mobile · 0 TS/lint**. Pendiente: deploy a producción + smoke tests.

**Fecha**: 2026-06-26 (auditoría Lote 1 — eliminación Intl/toLocaleString restantes, BUG-001–006) — Barrido proactivo de 6 crashs potenciales en Hermes que el barrido anterior (2026-06-25) no capturó porque se centró en `Intl.*` pero omitió `Number.prototype.toLocaleString()` y `Date.prototype.toLocaleDateString()`. **BUG-001** (`RankingItem.tsx`): 3× `entry.xp.toLocaleString()` → `formatNumber(entry.xp)` (2 en accessibilityLabel + 1 texto visible). **BUG-002** (`rankings.tsx`): 3× `(myRanking.xp ?? 0).toLocaleString()` → `formatNumber(..., i18n.language)` (2 en accessibilityLabel + 1 texto); `i18n` añadido a destructuring de `useTranslation` en `RankingsScreen`. **BUG-003** (`wrapped/[year].tsx`): 3× `totalXpGained.toLocaleString()` — 1 dentro de `buildShareText` (función fuera del componente, se añade param `lang: string` y se pasa `i18n.language` en la llamada) + 2 en StatCard `value`/`sub`. `formatNumber` añadido al import existente de `lib/formatTimeAgo`. **BUG-004** (`profile/[username].tsx`): 2× `compareData.xpDiff.toLocaleString()` → `formatNumber(compareData.xpDiff, i18n.language)`; la lógica de signo `+${...}` / valor negativo con guion se conserva intacta ya que `formatNumber` maneja el signo negativo. `formatNumber` importado; `i18n` añadido a destructuring. **BUG-005** (`profile.tsx`): `new Date(account.lastSyncedAt).toLocaleDateString()` — reemplazado por nueva función `formatFullDate(account.lastSyncedAt)`. `formatFullDate(dateStr, _lang?)` añadida a `lib/formatTimeAgo.ts`: divide el ISO datetime por `'T'` antes de partir por `'-'` para manejar timestamps completos sin `Date` getters dependientes de la zona horaria del dispositivo; `padStart(2, '0')` en día y mes; devuelve `DD/MM/AAAA` preservando el año. 6 tests nuevos: ISO datetime, fecha plana, dígito simple, fin de año, inicio de año, invariante del año. **BUG-006** (`UserCard.tsx`): 1× `user.xp.toLocaleString()` → `formatNumber(user.xp, i18n.language)`; `i18n` añadido a destructuring. **Efecto colateral en tests**: `RankingsScreen.test.tsx` tenía 3 assertions con `toLocaleString()` que en el entorno Jest/Node.js devolvían resultados inconsistentes (separador ES de punto para números grandes, sin separador para 1200) respecto a `formatNumber(..., 'en')` del componente (coma). Migradas a `formatNumber(value, 'en')` alineado con el mock de `react-i18next` (`language: 'en'`). **Convención reforzada en CLAUDE.md**: regla ahora cubre explícitamente `Number.prototype.toLocaleString()`, `Date.prototype.toLocaleDateString()` y `Date.prototype.toLocaleTimeString()`; se añade comando grep de verificación pre-commit. Tests: **462 mobile (+6 nuevos formatFullDate) · 632 API · 0 TS/lint** (100% mobile, sin tocar backend).

**Fecha**: 2026-06-25 (segunda ronda prueba cerrada — BUG-01 a MEJ-02 + assets de marca + hallazgo rareza PSN) — 5 correcciones derivadas del feedback de testers. BUG-01: crash `Intl.RelativeTimeFormat` en Android → `lib/formatTimeAgo.ts` extendida con `formatBirthDate`; 5 usos de Intl barridos. BUG-02: navegación rota en vinculación de plataformas → guard `canGoBack()` en las 3 pantallas + Alert de éxito añadido en PSN. MEJ-01: `@react-native-community/datetimepicker@8.6.0` integrado en registro → picker nativo con `maximumDate` al corte de 16 años; placeholder email actualizado en login/registro/forgot-password. BUG-03: botón rewarded ad — `disabled` extendido a `!isAdReady`; texto "Cargando anuncio..." cuando no está listo. MEJ-02: columna nombre amigo en comparación `w-9` → `minWidth:36,maxWidth:100`; nombres de logros centrados en `CompareRow`. Commit ec44b01: icono y splash reemplazados con assets finales de la marca (sin alfa, dimensiones correctas). Hallazgo rareza PSN: `trophyEarnedRate` (string `"2.9"`) y `trophyRare` (bucket 0-3) disponibles en psn-api — documentado en sección PSN y BACKLOG F46. Tests: **412 mobile · 632 API · 0 TS/lint** (tests actualizados, no añadidos netos).
