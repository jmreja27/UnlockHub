...
- **`router.back()` — guard obligatorio**: todo `router.back()` en `apps/mobile/` **debe** usar el hook `hooks/useSafeBack.ts` (`const safeBack = useSafeBack(); ... onPress={safeBack}`). Razón: sin historial en el stack (entrada por deep link, push notification o `router.replace` desde onboarding) `router.back()` saca al usuario de la app — regresión documentada como BUG-02 en vinculación de plataformas (2026-06-25) y BUG-009 a BUG-014 (2026-06-26). El hook llama `router.back()` si hay historial, o `router.replace('/(tabs)')` si no lo hay. **Nunca** escribir `router.back()` ni `if (router.canGoBack()) router.back()` inline en código nuevo — usar siempre el hook. Para detectar reincidencias en código nuevo: `grep -rn "router\.back()" apps/mobile/app apps/mobile/components`. Usos legacy pre-convención pendientes de barrido: `(auth)/forgot-password.tsx`, `(auth)/register.tsx`, `premium.tsx` (3×), `privacy.tsx`, `components/ComingSoon.tsx` — bajo riesgo (siempre tienen historial en sus flujos normales), se migrarán en el siguiente barrido.
