...
- **Caveat conocido (T135 en BACKLOG)**: correr `npm test --workspace=apps/api` con Docker local levantado + `.env` presente hace fallar 21 tests de `user.routes.test.ts` — el test no mockea `lib/prisma` y dependía implícitamente de que la conexión a BD fallara (fail-open de `authenticate.ts`). No es una regresión de producción, es fragilidad de aislamiento de tests preexistente que el entorno local expone. Fix pendiente (código, no aplicado en esta sesión de solo-docs): mockear `lib/prisma` en ese fichero.
