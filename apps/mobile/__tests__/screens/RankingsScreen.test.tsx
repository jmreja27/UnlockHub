...
const sampleEntries: RankingEntry[] = [
  { userId: 'u1', username: 'campeón', avatar: null, xp: 9000, rank: 1, countryCode: 'ES' },
...
describe('RankingsScreen', () => {
  ...
  it('muestra el skeleton durante la carga', () => {
    ...
    // En carga no hay botón de reintento ni lista
    expect(queryByRole('alert')).toBeNull();
  });

  it('muestra el título y mensaje de error cuando la carga falla', () => {
  ...
  it('muestra el botón de reintento con accessibilityLabel en estado de error', () => {
    setupMocks([], false, true);
  ...
  it('renderiza los ítems del ranking', () => {
  ...
  it('muestra la tarjeta de posición del usuario autenticado', () => {
  ...
  it('muestra "—" cuando el usuario no tiene posición en el ranking', () => {
    setupMocks(sampleEntries, false, false, { rank: null, xp: 100 }, 'u1');
    const { getByText } = render(<RankingsScreen />);
    expect(getByText('—')).toBeTruthy();
  });

  it('NO muestra la tarjeta de posición cuando no hay usuario autenticado', () => {
  ...
  it('NO muestra la tarjeta de posición cuando no hay datos de ranking propio', () => {
  ...
  it('resalta al usuario actual en la lista con "(Tú)"', () => {
    ...
    const { getByText } = render(<RankingsScreen />);
    expect(getByText('campeón (Tú)')).toBeTruthy();
  });

  it('la tarjeta de posición tiene el accessibilityLabel correcto con rank', () => {
    setupMocks(sampleEntries, false, false, { rank: 3, xp: 6000 }, 'u1');
    const { getByLabelText } = render(<RankingsScreen />);
    // t('rankings.my_position_aria', { rank: 3, xp: '6,000' }) → clave con 3 y 6000
    expect(getByLabelText('rankings.my_position_aria')).toBeTruthy();
  });

  it('la tarjeta de posición tiene accessibilityLabel de "sin ranking" cuando rank es null', () => {
