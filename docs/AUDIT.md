...
| A10 | Limpieza | 🔵 | 4 `console.log` de debug eliminados en `useRewardedAd.ts` — fuera del critical path del rewarded ad. **Adelantado desde S6.** 3 restantes (`profile.tsx:228`, `api.ts:180`) diferidos a S6. | `apps/mobile/hooks/useRewardedAd.ts`, `profile.tsx:228`, `api.ts:180` | ✅ S4 (4/7 eliminados · 3 restantes → S6) | S4 |
...
| A44 | Móvil / Cloudinary | 🔵 | Avatares y banners se sirven desde Cloudinary a resolución original (sin transformaciones). En la pantalla de perfil, el avatar se muestra a 96×96 pt (288×288 px en 3× screen); el banner a 100 % de ancho × 120 pt. Sin la transformación `c_fill,w_N,h_N,q_auto`, se descarga la imagen original que puede superar 500 KB por un thumbnail. `expo-image` la cachea tras la primera descarga, pero la descarga inicial es innecesariamente pesada para usuarios con conexiones lentas. **Propuesta**: añadir `getCloudinaryThumb(url, w, h)` en `lib/cloudinary.ts` que inyecte los parámetros de transformación en la URL antes de pasarla a `<Image source={{ uri }}>`. **Implementación**: `getCloudinaryThumb` reescribe la URL Cloudinary insertando `w_N/h_N/c_fill/q_auto/f_auto` en el path de transformación (`/upload/` → `/upload/w_N,h_N,c_fill,q_auto,f_auto/`) — sin cambiar lógica de upload ni contrato del backend. Coordinar que el backend devuelva la URL base sin crop. | `apps/mobile/app/(tabs)/profile.tsx`, `apps/mobile/components/LibraryGameCard.tsx` | 🔲 S5 | S4 |
| A45 | Móvil / Calidad | 🔵 | `useSyncStatus` configura `refetchInterval: 60_000` siempre activo para cualquier usuario autenticado. Incluso en periodos de total inactividad (sin sync corriendo) la app hace 1 petición/min a `/api/v1/sync/my-summary`. Impacto acotado (sigue siendo solo 1 req/min), pero una mejora sería usar `refetchInterval` dinámico: `false` (disable) cuando no hay sync activo, `60_000` cuando sí. El estado de sync activo se conoce desde `useSyncProgress`; circular dependency si se importa directamente. **Implementación**: `refetchInterval` acepta función `(q) => q.state.data?.isRunning ? 2_000 : false` — apaga el polling cuando no hay sync activo (`false` = disable en TanStack Query), activa a 2 s cuando sí hay. No requiere cambio de contrato de API ni campo `nextCheckIn`. | `apps/mobile/hooks/useSyncStatus.ts` | 🔲 S5 | S4 |
...
| **S5** | Mobile — seguridad de datos, almacenamiento | A44 🔲, A45 🔲 (+ análisis profundo de seguridad pendiente) |
| **S6** | Limpieza general (console.log, dead code, docs) | A2, A9, A10 (3 restantes), A12, A13, A27, A37–A40, A41 |
