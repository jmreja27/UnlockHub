...
| A57 | Hardening / Infraestructura | 🔵 | CWE-250 — `apps/api/Dockerfile` (raíz) y `apps/worker/Dockerfile` corrían el proceso como root dentro del contenedor. **Fix aplicado**: instalación/compilación (bcrypt@6 nativo, apk add/del) como root → `chown -R node:node /app` tras el último COPY → `USER node` (uid 1000) justo antes del CMD. **Fix adicional worker** (commit `e31fc95`): `ENV XDG_CACHE_HOME=/app/.cache` — tsx intentaba escribir su caché de transpilación en el directorio por defecto (propiedad de root) y fallaba en runtime bajo non-root sin esta variable. **Verificado en Railway (env production, 2026-06-27)**: API arrancó sin errores de módulo nativo + login real exitoso → bcrypt@6 carga y ejecuta hash/compare bajo uid 1000 ✅. Worker arrancó limpio (5 schedulers activos; auto-syncs restaurados count: 6); sync manual PSN ejecutado y completado ("Sync completado") → tsx transpila/escribe en `/app/.cache` bajo non-root sin fallo de permisos ✅. | `apps/api/Dockerfile`, `apps/worker/Dockerfile` | ✅ CERRADO S9 — Verificado en producción (2026-06-27). Commits: Dockerfiles (api+worker) + e31fc95 (cache tsx). Ver T98. | S8 / S9 |
