...
| A2 | Seguridad API | 🟠 | CVE `tar` (5 CVEs HIGH, rango `<=7.5.10`) — transitiva de `@mapbox/node-pre-gyp@1.0.11` (vía `bcrypt@5.1.1`). **Override imposible sin `--force`**: `@mapbox/node-pre-gyp@1.0.11` declara `tar@^6.1.11`; npm 11.12.1 no aplica overrides que crucen major-version boundaries. Probados: override plano, override anidado, eliminación de lock file + `npm install` desde cero, eliminación física de la cadena `tar/node-pre-gyp/bcrypt` — en todos los casos npm resuelve tar@6.2.1. **`npm audit fix --force` descartado** (introduciría breaking changes no auditados). **Riesgo aceptado**: tar se ejecuta exclusivamente durante `npm install` (bcrypt extrae su binario precompilado con `@mapbox/node-pre-gyp`), no se importa ni se invoca en el runtime del servidor. Superficie real: solo si un atacante controla el tarball de bcrypt descargado por npm (ataque de supply chain contra el registro npm bajo HTTPS + integridad SHA-512). **Alternativa futura**: actualizar `bcrypt@5.1.1 → 6.0.0` (usa `node-gyp-build`, elimina `@mapbox/node-pre-gyp` y `tar` por completo) — diferido a la siguiente auditoría de dependencias post-lanzamiento. | `tar` (transitiva de `@mapbox/node-pre-gyp`) | ✅ Riesgo aceptado documentado — override no aplicable sin `--force` | S6a |
