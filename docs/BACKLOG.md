...
| PL13 | ✅ Limpieza de usuarios de prueba antes de abrir a Producción pública | Script `scripts/cleanup-test-users.ts` creado. Borra (físico): User + UserAchievement + UserPoint + UserChallenge + ActivityEvent + Friendship + Notification + Subscription + PlatformAccount + RefreshToken + DeviceToken + PasswordResetToken + RankingSnapshot + AchievementGuide. Conserva: Game y Achievement (catálogo). Limpia Rankings Redis (ranking:global, ranking:global:*, ranking:platform:*). Opciones: `--dry-run` (preview sin borrar), `--preserve-username=TestUser99` (conserva cuenta de revisión de Google). Idempotente. Uso: `cd apps/api && DATABASE_URL="${DIRECT_URL}" REDIS_URL="redis://..." npx ts-node ../../scripts/cleanup-test-users.ts --preserve-username=TestUser99`. Ejecutar justo antes de promover a Producción pública. |
