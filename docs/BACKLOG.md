...
| PL20 | ⚙️ Smoke tests en device con build de release — bloque lanzamiento (8 días) | 3 escenarios obligatorios post-build: (1) Flujo UMP cerrado → banner aparece en Home (A49 — verificar en device real, no emulador: `consentResolved=false` bloquea render hasta que el formulario UMP se cierre). (2) Sync parcial con Steam >80 % cuota → aviso ámbar "Steam omitido" persiste en SyncStatusBar tras sync completado (A41). (3) Sync inicial + **segundo** sync incremental en cuenta real con >100 juegos Steam — verificar A51: primer sync procesa los 100 más recientes por `rtime_last_played`; segundo sync re-procesa los mismos 100 activos (no marca los omitidos como sincronizados-vacíos ni los pierde). |
| PL21 | ⚙️ Analítica lista para aprender — bloque lanzamiento | Confirmar que PostHog captura en producción: `platform_linked`, `sync_completed`, `wrapped_shared`, `premium_paywall_seen`. Definir las 2-3 métricas que decidirán si/cuándo reactivar premium: retención D7 + frecuencia syncs/semana (pesan más que instalaciones totales). Dashboard PostHog con estas métricas configurado ANTES del día de lanzamiento. Sin este paso, el lanzamiento no generará los datos necesarios para decidir cuándo activar `FEATURES.premium = true`. |
