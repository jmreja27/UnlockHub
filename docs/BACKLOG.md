...
| F21 | Ver logros de otros usuarios + comparativa | 🟡 Alta — Fase 4 — Desde perfil público: ver biblioteca de juegos del usuario con % completado. Al entrar en un juego: modo "Solo ver" (logros del visitado, locked/unlocked) y modo "Comparar" (dos columnas, tus logros vs los suyos). Respetar privacidad de F29 — solo mostrar si el perfil es público o el visitante es amigo. |
...
| F29 | Privacidad de perfil (3 niveles) | 🟡 Alta — Fase 4 — Nueva configuración en perfil propio: Público (aparece en rankings + logros visibles para todos), Solo amigos (no aparece en rankings + logros visibles solo para amigos aceptados), Privado (no aparece en rankings + logros no visibles para nadie). Al cambiar privacidad: actualizar sorted sets Redis en consecuencia. Nuevo campo User.profileVisibility enum PUBLIC\|FRIENDS_ONLY\|PRIVATE. Afecta a: perfil público, pantalla de logros de F21, rankings. |
