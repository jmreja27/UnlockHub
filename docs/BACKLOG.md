...
| PL19 | ⚙️ Bugs de código cerrados — solo falta nueva build + smoke test para promover a Producción. | Bugs corregidos: BUG-A (caché Redis tras unlink), BUG-B (edge-to-edge user-game), BUG-C (edge-to-edge profile tab), BUG-D (orden biblioteca usuario público). Auditoría completa sesión 70c — 23 issues detectados y corregidos. Segunda auditoría sesión 71 — 17 fixes AdMob lifecycle, i18n, queryKeys, socket guards. Sesión 72: avatar upload ✅, banner upload ✅, rewarded ad puntos ✅ (ruta incorrecta corregida), login con avatar/banner ✅. **Bugs de código: todos cerrados (T61-T65, T69-T73). Solo queda: (1) nueva EAS build, (2) smoke test sobre ella — login + registro + sync Steam/RA/PSN + biblioteca + rankings + perfil público + Wrapped + sin errores 5xx en Railway logs.** |
